...
@section('content')
<div class="row g-3 mb-4">@foreach([['Total Products',$totalItems,'primary'],['Total Stock In',number_format($totalInitial,2),'info'],['Total Sold',number_format($totalSold,2),'warning'],['Total Remaining Stock',number_format($totalRemaining,2),'success'],['Total Brands',$totalBrands,'secondary'],['Total Categories',$totalCategories,'dark']] as [$label,$value,$color])<div class="col-sm-6 col-xl-4"><div class="card p-4 border-start border-4 border-{{$color}}"><div class="text-secondary">{{$label}}</div><div class="display-6 fw-semibold">{{$value}}</div></div></div>@endforeach</div>
<div class="row g-4"><div class="col-xl-6"><div class="card p-3"><h2 class="h5">Stock By Category</h2><div style="height:320px"><canvas id="categoryChart"></canvas></div></div></div><div class="col-xl-6"><div class="card p-3"><h2 class="h5">Stock By Brand</h2><div style="height:320px"><canvas id="brandChart"></canvas></div></div></div></div>
@endsection
